...
function getDomesticDomainAllowListTmpFilePath() {
	return path.join(
		config.get().server.setting.userBasePath,
...
async function downloadDomesticDomainAllowListAsync() {
	...
	request(remoteFileUrl, (error, response, body) => {
		if (error) {
			log.error(
				`下载远程 domestic-domain-allowlist.txt 文件失败: ${remoteFileUrl}, error:`,
				error,
				", response:",
				response,
				", body:",
		...
		if (response && response.statusCode === 200) {
			if (body == null || body.length < 100) {
				...
				log.info(
					"下载远程 domestic-domain-allowlist.txt 文件成功:",
					remoteFileUrl,
				);
			}
			...
			try {
				...
				if (!fileTxt.includes("*.")) {
					log.error(
						`远程 domestic-domain-allowlist.txt 文件内容即不是base64格式，也不是要求的格式，url: ${remoteFileUrl}，body: ${body}`,
					);
					return;
				}
			}
			...
			log.error(
				`下载远程 domestic-domain-allowlist.txt 文件失败: ${remoteFileUrl}, response:`,
				response,
				", body:",
				body,
			);
...
function saveDomesticDomainAllowListFile(fileTxt) {
	...
	try {
		...
		log.error(
			"保存 domestic-domain-allowlist.txt 文件失败:",
			filePath,
			", error:",
			e,
		);
	...
	if (lastModifiedTime) {
		fs.stat(filePath, (err, _stats) => {
			...
			fs.utimes(filePath, lastModifiedTime, lastModifiedTime, (utimesErr) => {
				if (utimesErr) {
					log.error(
						"修改 domestic-domain-allowlist.txt 文件时间失败:",
						utimesErr,
					);
				} else {
					log.info(
						`'${filePath}' 文件的修改时间已更新为其最近更新时间 '${dateUtil.format(lastModifiedTime, false)}'`,
					);
				}
			});
		});
...
function getDomesticDomainAllowList() {
	...
	let fileAbsolutePath =
		config.get().proxy.domesticDomainAllowListFileAbsolutePath;
	if (
		!fileAbsolutePath &&
		config.get().proxy.autoUpdateDomesticDomainAllowList
	) {
		// 异步下载，下载成功后，下次系统代理生效
	...
	if (!fileAbsolutePath) {
		...
		if (fs.existsSync(tmpFilePath)) {
			...
			log.info(
				"读取已下载的 domestic-domain-allowlist.txt 文件:",
				fileAbsolutePath,
			);
		} else {
			// 如果临时文件不存在，则使用内置文件
			...
			fileAbsolutePath = path.join(
				__dirname,
				"../",
				config.get().proxy.domesticDomainAllowListFilePath,
			);
			log.info(
				"读取内置的 domestic-domain-allowlist.txt 文件:",
				fileAbsolutePath,
			);
		}
	} else {
		log.info(
			"读取自定义路径的 domestic-domain-allowlist.txt 文件:",
			fileAbsolutePath,
		);
	}
	...
	try {
		...
		log.error(
			`读取 domestic-domain-allowlist.txt 文件失败: ${fileAbsolutePath}, error:`,
			e,
		);
		return null;
	}
				...
				if (domesticDomainList && domesticDomainList.length > 0) {
					for (const domesticDomain of domesticDomainList) {
						if (proxyExcludeIpConfig[domesticDomain] !== false) {
							...
							log.info(
								"系统代理排除列表拼接国内域名时，跳过域名，系统代理将继续代理它:",
								domesticDomain,
							);
						}
					}
...
const executor = {
	async windows(exec, params = {}) {
		...
		if (ip != null) {
			...
			// 设置代理，同时设置排除域名
			try {
				require("@starknt/sysproxy").triggerManualProxyByUrl(
					true,
					proxyAddr,
					excludeIpStr,
					true,
				...
				log.info(
					`设置windows系统代理成功: ${proxyAddr} ......(省略排除IP列表)`,
				);
			} catch (e1) {
				log.warn(
					"设置windows系统代理失败：执行 `@starknt/sysproxy` 失败，现尝试通过执行 `sysproxy.exe global ...` 来设置系统代理！\r\n捕获的异常:",
					e1,
				);
				...
				try {
					...
					log.info(
						`设置windows系统代理成功，执行的命令：${proxyPath} ${execFun} ${proxyAddr} ......(省略排除IP列表)`,
					);
				} catch (e2) {
					log.error(
						`设置windows系统代理失败，执行的命令：${proxyPath} ${execFun} ${proxyAddr} ......(省略排除IP列表), error:`,
						e2,
					);
					throw e1; // 将上面的异常抛出
				}
			...
			if (setEnv) {
				...
				try {
					await exec(
						`echo '设置环境变量 HTTPS_PROXY${config.get().proxy.proxyHttp ? "、HTTP_PROXY" : ""}'`,
					);

					log.info(
						`开启系统代理的同时设置环境变量：HTTPS_PROXY = "http://${ip}:${port}/"`,
					);
					await exec(`setx HTTPS_PROXY "http://${ip}:${port}/"`);

					if (config.get().proxy.proxyHttp) {
						log.info(
							`开启系统代理的同时设置环境变量：HTTP_PROXY = "http://${ip}:${port - 1}/"`,
						);
			...
			try {
				...
				log.error(
					"关闭windows系统代理失败：执行 `@starknt/sysproxy` 失败，现尝试通过执行 `sysproxy.exe set 1` 来关闭系统代理！\r\n捕获的异常:",
					e1,
				);
				...
				try {
					...
					log.error(
						"关闭windows系统代理失败，执行的命令：sysproxy.exe set 1, error:",
						e2,
					);
					throw e1; // 将上面的异常抛出
				}
	...
	async linux(exec, params = {}) {
		...
		if (ip != null) {
			...
			const excludeIpStr = getProxyExcludeIpStr("', '");
			setProxyCmd.push(
				`gsettings set org.gnome.system.proxy ignore-hosts "['${excludeIpStr}']"`,
			);

			await exec(setProxyCmd);
		} else {
			// 关闭代理
			const setProxyCmd = ["gsettings set org.gnome.system.proxy mode none"];
			await exec(setProxyCmd);
		}
	},
	async mac(exec, params = {}) {
		// exec = _exec
		let wifiAdaptor = await exec(
			"sh -c \"networksetup -listnetworkserviceorder | grep `route -n get 0.0.0.0 | grep 'interface' | cut -d ':' -f2` -B 1 | head -n 1 \"",
		);
		...
		if (ip != null) {
			...
			// https
			await exec(
				`networksetup -setsecurewebproxy "${wifiAdaptor}" ${ip} ${port}`,
			);
			// http
			if (config.get().proxy.proxyHttp) {
				await exec(
					`networksetup -setwebproxy "${wifiAdaptor}" ${ip} ${port - 1}`,
				);
			} else {
			...
			const excludeIpStr = getProxyExcludeIpStr('" "');
			await exec(
				`networksetup -setproxybypassdomains "${wifiAdaptor}" "${excludeIpStr}"`,
			);
