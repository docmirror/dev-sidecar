...
const configApi = {
	...
	async downloadRemoteConfig() {
		...
		await configApi.doDownloadRemoteConfig(
			remoteConfig.personalUrl,
			"_personal",
		);
	},
	doDownloadRemoteConfig(remoteConfigUrl, suffix = "") {
		...
		return new Promise((resolve, reject) => {
			...
			request(remoteConfigUrl, { headers }, (error, response, body) => {
				if (error) {
					log.error(
						`下载远程配置失败: ${remoteConfigUrl}, error:`,
						error,
						", response:",
						response,
						", body:",
				...
				if (response && response.statusCode === 200) {
					...
					try {
						...
						log.error(
							`远程配置内容格式不正确, url: ${remoteConfigUrl}, body: ${body}`,
						);
						remoteConfig = null;
					}
					...
					log.error(
						`下载远程配置失败: ${remoteConfigUrl}, response:`,
						response,
						", body:",
						body,
					);
