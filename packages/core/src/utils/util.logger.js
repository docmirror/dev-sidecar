...
const appenderConfig = {
	...
	backups:
		Math.ceil(configFromFiles.app.keepLogFileCount) ||
		defaultConfig.app.keepLogFileCount, // 保留日志文件数
	maxLogSize: Math.ceil(
		(configFromFiles.app.maxLogFileSize || defaultConfig.app.maxLogFileSize) *
			1024 *
			1024 *
			(configFromFiles.app.maxLogFileSizeUnit === "GB" ? 1024 : 1),
	), // 目前单位只有GB和MB
};
...
function log4jsConfigure(categories) {
	...
	log.info(
		`设置日志配置完成，进程ID: ${process.pid}，categories：[${categories}]，config:`,
	...
	getLogger(category) {
		...
		if (category === "core" || category === "gui") {
			...
			if (log == null) {
				...
				log.error(
					`当前进程已经设置过日志配置，无法再设置 "${category}" 的配置，先临时返回 "${log.category}" 的 log 进行日志记录。如果与其他类型的日志在同一进程中写入，请参照 core 和 gui 一起配置`,
				);
			}
