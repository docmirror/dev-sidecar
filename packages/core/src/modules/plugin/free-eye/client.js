...
async function loadAllTests(globalConfig, testsDir) {
	...
	const files = fs
		...
		.filter((file) => file.endsWith(".js") && file !== "__init__.js");
...
async function runTests(options = {}) {
	...
	const preferredConfigPath =
		configPath && configPath.length > 0
			? path.isAbsolute(configPath)
				? configPath
				: path.join(PLUGIN_ROOT, configPath)
			: null;
	const fallbackConfigPath = path.join(PLUGIN_ROOT, "config.json");

	const configCandidates = Array.from(
		new Set([preferredConfigPath, fallbackConfigPath].filter(Boolean)),
	);
	...
	for (const candidatePath of configCandidates) {
		try {
			...
			lastError = new Error(
				`Error reading config file (${candidatePath}): ${error.message}`,
			);
		}
	}
	...
	console.log(
		`Loaded ${todoTests.length} tests: ${todoTests
			.map((t) => t.getTestTag())
			.join(" ")}`,
	);
	...
	while (todoTests.length > 0) {
		...
		if (testGroup.skipReason === null) {
			...
			printHeader(
				`${testTag} Results: (done in ${testTime.toFixed(3)}s)`,
				true,
			);
