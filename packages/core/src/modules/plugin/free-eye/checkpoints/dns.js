...
class DnsTester extends TestGroup {
	...
	logResults() {
		...
		for (const [family, results] of Object.entries(this.results)) {
			...
			const blockOkCnt = blockList.reduce(
				(sum, host) => sum + (results[host] || 0),
				0,
			);
			...
			if (blockOkCnt < blockTotal) {
				censors.push(
					`DNS blocking: ${blockTotal - blockOkCnt}/${blockTotal} blocked`,
				);
			}

			const blockPoisonCnt = blockList.reduce(
				(sum, host) => sum + (results[this.testPrefix + host] || 0),
				0,
			);
			if (blockPoisonCnt > 0) {
