	...
	logResults() {
		...
		for (const [family, portRes] of Object.entries(this.results)) {
			...
			for (const [port, results] of Object.entries(portRes)) {
				...
				const allowOkCnt = allowList.reduce(
					(sum, addr) => sum + (results[addr] === null ? 1 : 0),
					0,
				);
				const allowTotal = allowList.length;
				let resIcon;
				if (allowOkCnt === allowTotal) {
					// can connect
					resIcon = getResultIcon(true);
				} else if (allowOkCnt === 0) {
					// can't connect
					resIcon = getResultIcon(false);
				} else {
					// test inconclusive
					resIcon = getResultIcon(
						null,
						`connected ${allowOkCnt}/${allowTotal}`,
					);
				}
				...
				const timeoutCnt = blockList.reduce(
					(sum, addr) => sum + (results[addr] === "timeout" ? 1 : 0),
					0,
				);
				if (timeoutCnt > 0) {
					censors.push(
						`Blocked ${dstTag} handshake timeouts: ${timeoutCnt}/${blocksTotal} timeouts`,
					);
				}

				const errorCnt = blockList.reduce(
					(sum, addr) => sum + (results[addr] === "error" ? 1 : 0),
					0,
				);
				if (errorCnt > 0) {
					censors.push(
						`Blocked ${dstTag} handshake errors: ${errorCnt}/${blocksTotal} errors`,
					);
				}
			}
