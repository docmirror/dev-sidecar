...
function matchHostname(hostMap, hostname, action) {
	...
	if (hostMap == null) {
		log.warn(
			`matchHostname: ${action}: '${hostname}' Not-Matched, hostMap is null`,
		);
		return null;
	}
	if (hostMap.origin == null) {
		log.warn(
			`matchHostname: ${action}: '${hostname}' Not-Matched, hostMap.origin is null`,
		);
		return null;
	}
	...
	if (value != null) {
		log.info(
			`matchHostname: ${action}: '${hostname}' -> { "${hostname}": ${JSON.stringify(value)} }`,
		);
		return value; // 快速匹配成功
	}
	// 域名快速匹配：三种前缀通配符匹配
	...
	for (const regexp in hostMap) {
		...
		if (hostname.match(regexp)) {
			...
			log.info(
				`matchHostname: ${action}: '${hostname}' -> { "${regexp}": ${JSON.stringify(value)} }`,
			);
			return value;
		}
	}
...
function matchHostnameAll(hostMap, hostname, action) {
	...
	if (hostMap.origin == null) {
		log.warn(
			`matchHostname-all: ${action}: '${hostname}', hostMap.origin is null`,
		);
		return null;
	}
	...
	for (const regexp in hostMap) {
		...
		if (matched) {
			value = hostMap[regexp];
			log.debug(
				`matchHostname-one: ${action}: '${hostname}' -> { "${regexp}": ${JSON.stringify(value)} }`,
			);
	...
	if (value) {
		log.debug(
			`matchHostname-one: ${action}: '${hostname}' -> { "*${hostname}": ${JSON.stringify(value)} }`,
		);
		values = merge(values, value);
	}
	// 优先级：3
	...
	if (!lodash.isEmpty(values)) {
		...
		log.info(
			`matchHostname-all: ${action}: '${hostname}':`,
