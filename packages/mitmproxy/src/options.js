...
} = require("./lib/proxy/middleware/overwall");
...
module.exports = (serverConfig) => {
	const intercepts = matchUtil.domainMapRegexply(
		buildIntercepts(serverConfig.intercepts),
	);
	const whiteList = matchUtil.domainMapRegexply(serverConfig.whiteList);
	const timeoutMapping = matchUtil.domainMapRegexply(
		serverConfig.setting.timeoutMapping,
	);

	const dnsMapping = handleDnsMapping(
		serverConfig.dns.mapping,
		serverConfig.dns.familyMapping || {},
	);
	const setting = serverConfig.setting;

	if (!setting.script.dirAbsolutePath) {
		setting.script.dirAbsolutePath = path.join(
			setting.rootDir,
			setting.script.defaultDir,
		);
	}
	...
	const options = {
		...
		setting,
		compatibleConfig: {
			connect: serverConfig.compatible
				? matchUtil.domainMapRegexply(serverConfig.compatible.connect)
				: {},
			request: serverConfig.compatible
				? matchUtil.domainMapRegexply(serverConfig.compatible.request)
				: {},
		},
		middlewares,
		sslConnectInterceptor: (req, _cltSocket, _head) => {
			...
			// 配置了白名单的域名，将跳过代理
			const inWhiteList = !!matchUtil.matchHostname(
				whiteList,
				hostname,
				"in whiteList",
			);
			...
			// 配置了拦截的域名，将会被代理
			const matched = matchUtil.matchHostname(
				intercepts,
				hostname,
				"matched intercepts",
			);
		...
		createIntercepts: (context) => {
			...
			const interceptOpts = matchUtil.matchHostnameAll(
				intercepts,
				rOptions.hostname,
				"get interceptOpts",
			);
			...
			if (!interceptOpts._pathCache) {
				...
				Object.defineProperty(interceptOpts, "_pathCache", {
					value: cache,
					enumerable: false,
					configurable: true,
				});
			...
			for (const regexp in interceptOpts) {
				...
				if (interceptOpt.exclusions) {
					...
					try {
						...
						if (exclusions) {
							for (const exclusion of exclusions) {
								if (matchUtil.isMatched(rOptions.path, exclusion)) {
									log.debug(
										`拦截器配置排除了path：${rOptions.protocol}//${rOptions.hostname}:${rOptions.port}${rOptions.path}, exclusion: '${exclusion}', interceptOpt:`,
										interceptOpt,
									);
									isExcluded = true;
								}
						...
						log.error(
							`判断拦截器是否排除当前path时出现异常, path: ${rOptions.path}, interceptOpt:`,
							interceptOpt,
							", error:",
							e,
						);
				...
				log.debug(
					`拦截器匹配path成功：${rOptions.protocol}//${rOptions.hostname}:${rOptions.port}${rOptions.path}, regexp: ${regexp}, interceptOpt:`,
					interceptOpt,
				);
					...
					if (impl.is?.(interceptOpt)) {
						...
						if (matchedInterceptOpt) {
							if (matchedInterceptOpt.order >= (interceptOpt.order || 0)) {
								log.warn(
									`duplicate interceptor: ${impl.name}, hostname: ${rOptions.hostname}`,
								);
								continue;
							}
							action = "replace";
						...
						matchInterceptsOpts[impl.name] = {
							...
							index:
								action === "replace"
									? matchedInterceptOpt.index
									: matchIntercepts.length - 1,
						};
					}
				}
			}
