...
module.exports = {
	initDNS(dnsProviders, preSetIpList) {
		...
		for (const provider in dnsProviders) {
			...
			if (type === "https" || type === "doh" || type === "dns-over-https") {
				...
				if (type === "tls" || type === "dot" || type === "dns-over-tls") {
					// 基于 tls
					dnsMap[provider] = new DNSOverTLS(
						provider,
						conf.cacheSize,
						preSetIpList,
						server,
					...
					// 基于 tcp
					dnsMap[provider] = new DNSOverTCP(
						provider,
						conf.cacheSize,
						preSetIpList,
						server,
					...
					// 基于 udp
					dnsMap[provider] = new DNSOverUDP(
						provider,
						conf.cacheSize,
						preSetIpList,
						server,
		...
		log.info(
			`设置SNI默认使用的DNS为 '${dnsMap.ForSNI.dnsName}'（注：当某个域名配置了SNI但未配置DNS时，将默认使用该DNS）`,
		);
	...
	getDNSAndFamily(dnsConfig, hostname) {
		// 1. 匹配 预设IP配置
		const hostnamePreSetIpList = matchUtil.matchHostname(
			dnsConfig.preSetIpList,
			hostname,
			"matched preSetIpList(getDNSAndFamily)",
		);
		...
		// 2. 读取域名对应的DNS配置
		const dnsData = matchUtil.matchHostname(
			dnsConfig.mapping,
			hostname,
			"get dns data",
		);
		...
		if (
			dnsData.dnsName === "usa" &&
			dnsConfig.dnsMap.usa == null &&
			dnsConfig.dnsMap.cloudflare != null
		) {
