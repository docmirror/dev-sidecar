	...
	for (let i = 0; i < proxyRes.rawHeaders.length; i += 2) {
		...
		if (newHeaderValue) {
			if (newHeaderValue !== proxyRes.rawHeaders[i + 1]) {
				...
				if (newHeaderValue === REMOVE) {
					// 由于拦截配置中不允许配置null，会被删，所以配置一个 "[remove]"，当作删除响应头的意思
...
module.exports = {
	...
	responseIntercept(
		...
		if (responseReplaceConfig.doDownload || rOptions.doDownload) {
			const filename =
				(FILENAME_RE.exec(rOptions.path) || [])[1] || "UNKNOWN_FILENAME";
			// 设置文件下载响应头
			replaceHeaders["content-disposition"] =
