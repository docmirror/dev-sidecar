...
function getScript(key, _script) {
...
module.exports = {
	...
	responseIntercept(
		...
		// github特殊处理
		if (
			rOptions.hostname === "github.com" &&
			rOptions.headers["turbo-frame"] === "repo-content-turbo-frame"
		) {
			return;
		}
		...
		try {
			...
			if (rOptions.hostname === "github.com") {
				tags += `\r\n\t${getScriptByUrlOrPath("https://github.githubassets.com/assets/fakefile.js")}`;
			}

			res.setHeader("DS-Script-Interceptor", "true");
			log.info(
				`script response intercept: insert script ${rOptions.protocol}//${rOptions.hostname}:${rOptions.port}${rOptions.path}`,
				", head:",
				tags,
			);
			return {
	...
	handleScriptInterceptConfig(intercepts) {
		...
		for (const hostnamePattern in intercepts) {
			...
			const handleScriptUrl = (scriptUrl, name, replaceScriptUrlFun) => {
				if (
					scriptUrl.indexOf("https:") === 0 ||
					scriptUrl.indexOf("http:") === 0
				) {
					// 绝对地址
			...
			for (const pathPattern in hostnameConfig) {
				...
				// 处理 script 配置
				if (
					typeof pathConfig.script === "object" &&
					pathConfig.script.length > 0
				) {
			...
			if (!lodash.isEmpty(scriptProxy)) {
				for (const scriptKey in scriptProxy) {
					if (scriptKey.indexOf(SCRIPT_PROXY_URL_PRE) === 0) {
						...
						log.debug(
							`域名 '${hostnamePattern}' 拦截配置中，新增伪脚本地址的代理配置:`,
						...
						log.info(
							`域名 '${hostnamePattern}' 拦截配置中，新增目标脚本地址的响应头替换配置:`,
							JSON.stringify(obj, null, "\t"),
						);
					}
				}
