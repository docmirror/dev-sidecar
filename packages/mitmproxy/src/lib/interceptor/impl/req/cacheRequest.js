...
function getMaxAge(interceptOpt) {
	// 秒
	if (
		interceptOpt.cacheSeconds > 0 ||
		interceptOpt.cacheMaxAge > 0 ||
		interceptOpt.cache > 0
	) {
		return (
			interceptOpt.cacheSeconds ||
			interceptOpt.cacheMaxAge ||
			interceptOpt.cache
		);
	}
	// 分钟
...
module.exports = {
	...
	requestIntercept(context, interceptOpt, req, res, _ssl, _next) {
		...
		if (
			pragma &&
			(pragma.includes("no-cache") || pragma.includes("no-store"))
		) {
			return; // 当前请求指定要禁用缓存，跳过当前拦截器
		}

		// 最近编辑时间
		const lastModifiedTime = getLastModifiedTimeFromIfModifiedSince(
			rOptions,
			log,
		);
