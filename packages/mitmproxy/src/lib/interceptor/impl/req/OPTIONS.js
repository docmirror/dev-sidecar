...
module.exports = {
	...
	requestIntercept(context, interceptOpt, req, res, ssl, next) {
		...
		const allowHeaders =
			rOptions.headers["access-control-request-headers"] ||
			readConfig(interceptOpt.optionsAllowHeaders, defaultAllowHeaders);
		const allowMethods =
			rOptions.headers["access-control-request-method"] ||
			readConfig(interceptOpt.optionsAllowMethods, defaultAllowMethods);
		...
		const headers = {
			...
			"Access-Control-Max-Age":
				interceptOpt.optionsMaxAge > 0 ? interceptOpt.optionsMaxAge : 2592000, // 默认有效一个月
			Date: new Date().toUTCString(),
		};

		// 判断是否允许
		if (
			interceptOpt.optionsCredentials !== false &&
			interceptOpt.optionsCredentials !== "false"
		) {
