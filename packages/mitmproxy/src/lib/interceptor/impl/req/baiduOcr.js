...
function getConfig(interceptOpt, tryCount, log) {
	...
	let config;
	if (
		Array.isArray(interceptOpt.baiduOcr) &&
		interceptOpt.baiduOcr.length > 0
	) {
...
module.exports = {
	...
	requestIntercept(context, interceptOpt, req, res, _ssl, next) {
		...
		if (!configResult) {
			...
			res.write(
				'{"error_code": 99917, "error_msg": "dev-sidecar中，未配置百度云账号，或所有百度云账号的免费额度都已用完！！！"}',
			);
			res.end();
			return true;
		}
		...
		if (!config.id || !config.ak || !config.sk) {
			res.writeHead(200, headers);
			res.write(
				'{"error_code": 999500, "error_msg": "dev-sidecar中，baiduOcr的 id 或 ak 或 sk 配置为空"}',
			);
			res.end();
			return true;
		}

		headers["DS-Interceptor"] =
			`baiduOcr: id=${config.id}, api=${api}, account=${config.account}`;
			...
			.then((result) => {
				if (result.error_code != null) {
					...
					if (result.error_code === 17) {
						...
						log.error(
							`当前百度云账号的接口 ${api}，已达到当日调用次数上限，暂时禁用它，明天会自动放开:`,
							config,
						);
					}
				} else {
