...
module.exports = function createRequestHandler(
	...
	return function requestHandler(req, res, ssl) {
		...
		const rOptions = commonUtil.getOptionsFromRequest(
			req,
			ssl,
			externalProxy,
			setting,
				...
				try {
					...
					if (reqIncpts && reqIncpts.length > 0) {
						for (const reqIncpt of reqIncpts) {
							...
							const goNext = reqIncpt.requestIntercept(
								context,
								req,
								res,
								ssl,
		...
		function countSlow(isDnsIntercept, reason) {
			if (
				...
				log.error(
					`记录ip失败次数，用于优选ip！ hostname: ${hostname}, ip: ${ip}, reason: ${reason}, dns: ${dns.dnsName}`,
				);
			}
			...
			if (counter != null) {
				...
				log.error(
					`记录Proxy请求失败次数，用于切换备选域名！ hostname: ${counter.value}, reason: ${reason}, counter.count:`,
					counter.count,
				);
			}
		}
			...
			return new Promise((resolve, reject) => {
				...
				function onFree() {
					...
					log.info(
						"发起代理请求:",
						url,
						rOptions.servername ? `, sni: ${rOptions.servername}` : "",
						", headers:",
						jsonApi.stringify2(rOptions.headers),
					);
					...
					if (dnsConfig?.dnsMap) {
						...
						if (!dnsAndFamily && rOptions.servername) {
							...
							if (dns) {
								dnsAndFamily = { dns };
								log.info(
									`域名 ${rOptions.hostname} 在dns中未配置，但使用了 sni: ${rOptions.servername}, 必须使用dns，现默认使用 '${dnsAndFamily.dnsName}' DNS.`,
								);
							} else {
								log.warn(
						...
						if (dnsAndFamily) {
							rOptions.lookup = dnsLookup.createLookupFunc(
								res,
								dnsAndFamily,
								"request url",
								url,
							...
							log.debug(
								`域名 ${rOptions.hostname} DNS: ${dnsAndFamily.dns.dnsName}, family: ${rOptions.family || 4}`,
							);
					...
					res.setHeader(
						"DS-Proxy-Request",
						`${rOptions.protocol}//${rOptions.hostname}:${rOptions.port}${req.url}`,
					);

					// 自动兼容程序：2
					if (rOptions.agent) {
						const compatibleConfig = compatible.getRequestCompatibleConfig(
							rOptions,
							rOptions.compatibleConfig,
						);
						if (
							compatibleConfig &&
							compatibleConfig.rejectUnauthorized != null &&
							rOptions.agent.options.rejectUnauthorized !==
								compatibleConfig.rejectUnauthorized
						) {
							if (
								compatibleConfig.rejectUnauthorized === false &&
								rOptions.agent.unVerifySslAgent
							) {
								log.info(
									`【自动兼容程序】${rOptions.hostname}:${rOptions.port}: 设置 'rOptions.agent.options.rejectUnauthorized = ${compatibleConfig.rejectUnauthorized}'`,
								);
					...
					proxyReq = (rOptions.protocol === "https:" ? https : http).request(
						...
						(proxyRes) => {
							...
							if (rOptions.protocol === "https:") {
								log.info(
									`代理请求返回: 【${proxyRes.statusCode}】${url}, cost: ${cost} ms`,
								);
							} else {
								log.info(
									`请求返回: 【${proxyRes.statusCode}】${url}, cost: ${cost} ms`,
								);
							}
							...
							if (cost > MAX_SLOW_TIME) {
								countSlow(
									isDnsIntercept,
									`代理请求成功但太慢, cost: ${cost} ms > ${MAX_SLOW_TIME} ms`,
								);
							}
					...
					proxyReq.on("error", (e) => {
						const cost = Date.now() - start;
						log.error(
							`代理请求错误: ${url}, cost: ${cost} ms, error:`,
							e,
							", rOptions:",
							jsonApi.stringify2(rOptions),
						);
					...
					proxyReq.on("aborted", () => {
						...
						if (cost > MAX_SLOW_TIME) {
							countSlow(
								isDnsIntercept,
								`代理请求被取消，且请求太慢, cost: ${cost} ms > ${MAX_SLOW_TIME} ms`,
							);
						}
				...
				try {
					if (resIncpts && resIncpts.length > 0) {
						...
						for (const resIncpt of resIncpts) {
							const append = resIncpt.responseIntercept(
								context,
								req,
								res,
								proxyReq,
						...
						InsertScriptMiddleware.responseInterceptor(
							req,
							res,
							proxyReq,
							proxyRes,
