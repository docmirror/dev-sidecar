...
module.exports = function createConnectHandler(
	...
	return function connectHandler(req, cltSocket, head, ssl) {
		...
		if (isSslConnect(sslConnectInterceptors, req, cltSocket, head)) {
			...
			fakeServerCenter
				.getServerPromise(hostname, port, ssl, compatibleConfig)
				.then(
					(serverObj) => {
						log.info(
							`----- fakeServer connect: ${localIP}:${serverObj.port} ➜ ${req.url} -----`,
						);
						connect(
							req,
							cltSocket,
					...
					(e) => {
						log.error(
							`----- fakeServer getServerPromise error: ${hostname}:${port}, error:`,
							e,
						);
					},
				)
			...
			log.info(
				`不拦截请求，直连目标服务器: ${hostname}:${port}, headers:`,
				jsonApi.stringify2(req.headers),
			);
...
function connect(
	...
	// 用于记录日志
	const connectInfo = isDirect
		? hostport
		: `fakeServer: ${hostport}, target: ${target}`;
	...
	try {
		...
		if (printDebugLog) {
			...
			cltSocket.on("connectionAttempt", (ip, port, family) => {
				log.debug(
					`【cltSocket connectionAttempt】${ip}:${port}: ${connectInfo}, family:`,
					family,
				);
			});
			cltSocket.on("connectionAttemptFailed", (ip, port, family) => {
				log.debug(
					`【cltSocket connectionAttemptFailed】${ip}:${port}: ${connectInfo}, family:`,
					family,
				);
			});
			cltSocket.on("connectionAttemptTimeout", (ip, port, family) => {
				log.debug(
					`【cltSocket connectionAttemptTimeout】${ip}:${port}: ${connectInfo}, family:`,
					family,
				);
			});
		...
		proxySocket.on("timeout", () => {
			...
			cltSocket.write(
				"HTTP/1.1 408 Proxy connect timeout\r\n" +
					"Proxy-agent: dev-sidecar\r\n" +
					"\r\n",
			);
			cltSocket.end();
			...
			if (
				...
				log.error(
					`记录ip失败次数，用于优选ip！ hostname: ${hostname}, ip: ${ip}, reason: ${errorMsg}, dns: ${dns.dnsName}`,
				);
			}
		});
		proxySocket.on("error", (e) => {
			...
			cltSocket.write(
				`HTTP/1.1 400 Proxy connect error: ${e.message}\r\n` +
					"Proxy-agent: dev-sidecar\r\n" +
					"\r\n",
			);
			cltSocket.end();
		...
		if (printDebugLog) {
			...
			proxySocket.on("connectionAttempt", (ip, port, family) => {
				log.debug(
					`【proxySocket connectionAttempt】${ip}:${port}, family:`,
					family,
				);
			});
			proxySocket.on("connectionAttemptFailed", (ip, port, family) => {
				log.debug(
					`【proxySocket connectionAttemptFailed】${ip}:${port}, family:`,
					family,
				);
			});
			proxySocket.on("connectionAttemptTimeout", (ip, port, family) => {
				log.debug(
					`【proxySocket connectionAttemptTimeout】${ip}:${port}, family:`,
					family,
				);
			});
