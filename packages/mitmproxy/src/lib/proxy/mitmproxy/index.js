	...
	createProxy(
		...
		const serverListen = (server, ssl, port, host) => {
			server.listen(port, host, () => {
				log.info(
					`dev-sidecar启动 ${ssl ? "https" : "http"} 端口: ${host}:${port}`,
				);
				server.on("request", (req, res) => {
					if (printDebugLog) {
						log.debug(
							`【server request, ssl: ${ssl}】\r\n----- req -----\r\n`,
							req,
							"\r\n----- res -----\r\n",
							res,
						);
				...
				server.on("clientError", (err, cltSocket) => {
					...
					log.error(
						`【server clientError, ssl: ${ssl}】socket.localPort = ${cltSocket.localPort}\r\n`,
						err,
					);
				...
				if (printDebugLog) {
					...
					server.on("checkExpectation", (req, res) => {
						log.debug(
							`【server checkExpectation, ssl: ${ssl}】\r\n----- req -----\r\n`,
							req,
							"\r\n----- res -----\r\n",
							res,
						);
