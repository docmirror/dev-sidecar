...
function matched(hostname, overWallTargetMap) {
	...
	if (ret1) {
		...
		log.debug(
			`域名 ${hostname} 的overwall配置为 false，跳过增强功能，即使它在 pac.txt 里`,
		);
		return null;
	}
	...
	if (ret && ret.indexOf("PROXY ") === 0) {
		log.info(
			`matchHostname: matched overwall: '${hostname}' -> '${ret}' in pac.txt`,
		);
		return "in pac.txt";
	} else {
		log.debug(
			`matchHostname: matched overwall: Not-Matched '${hostname}' -> '${ret}' in pac.txt`,
		);
		return null;
	}
}
			...
			fs.utimes(
				...
				(utimesErr) => {
					if (utimesErr) {
						...
						log.info(
							`'${pacFilePath}' 文件的修改时间已更新为其最近更新时间 '${dateUtil.format(lastModifiedTime, false)}'`,
						);
					}
				},
			);
...
async function downloadPacAsync(pacConfig) {
	...
	request(remotePacFileUrl, (error, response, body) => {
		if (error) {
			log.error(
				`下载远程 pac.txt 文件失败: ${remotePacFileUrl}, error:`,
				error,
				", response:",
				response,
				", body:",
		...
		if (response && response.statusCode === 200) {
			...
			if (!pacTxt.includes("!---------------------EOF")) {
				try {
					...
					log.error(
						`远程 pac.txt 文件内容即不是base64格式，也不是要求的格式，url: ${remotePacFileUrl}，body: ${body}`,
					);
					return;
				}
			}
			...
			log.error(
				`下载远程 pac.txt 文件失败: ${remotePacFileUrl}, response:`,
				response,
				", body:",
				body,
			);
