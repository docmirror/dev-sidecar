...
module.exports = class CertAndKeyContainer {
	constructor({ maxLength = DEFAULT_MAX_LENGTH, caCert, caKey }) {
		// 缓存键：dnsName
	...
	getCertPromise(hostname, port, dnsName, mappingHostNames) {
		...
		if (cached) {
			log.debug(
				`Load fakeCertPromise from cache, hostname: ${hostname}:${port}, certPromiseObj: {"mappingHostNames":${JSON.stringify(cached.mappingHostNames)}}`,
			);
			return cached.promise;
		}

		log.info(
			`【CreateFakeCertificate】dnsName: ${dnsName}, hostname: ${hostname}:${port}`,
		);
		const promise = tlsUtils.createFakeCertificateByDomain(
			this.caKey,
			this.caCert,
			dnsName,
			mappingHostNames,
