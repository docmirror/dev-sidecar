...
const IPv4_RE =
	/\b(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d|\d)(?:\.(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d|\d)\b){3}/;
...
module.exports = class FakeServersCenter {
		...
		this.cache = new LRUCache({
			...
			dispose: (evictServerPromiseObj, evictDnsName) => {
				try {
					...
					log.error(
						`旧fake服务缓存被移除，但停止服务失败，${evictDnsName} ->`,
						evictServerPromiseObj,
						`, error:`,
						e,
					);
				}
	...
	getServerPromise(hostname, port, ssl, manualCompatibleConfig) {
		if (port === 443 || port === 80) {
			...
			// 自动兼容程序：1
			const compatibleConfig = compatible.getConnectCompatibleConfig(
				hostname,
				port,
				manualCompatibleConfig,
			);
		...
		if (cachedServerObj) {
			log.debug(
				`Load fakeServerPromise from cache, hostname: ${hostname}:${port}, ssl: ${ssl}, serverPromiseObj: {"ssl":${cachedServerObj.ssl},"port":${cachedServerObj.port},"mappingHostNames":${JSON.stringify(cachedServerObj.mappingHostNames)}}`,
			);
			return cachedServerObj.promise;
		}

		log.info(
			`getServerPromise, hostname: ${hostname}:${port}, ssl: ${ssl}, protocol: ${ssl ? "https" : "http"}`,
		);
		...
		const promise = new Promise((resolve, reject) => {
			(async () => {
				...
				log.info(
					`【CreateFakeServer】hostname: ${hostname}:${port}, ssl: ${ssl}, protocol: ${ssl ? "https" : "http"}`,
				);
				...
				fakeServer.on("request", (req, res) => {
					if (printDebugLog) {
						log.debug(
							`【fakeServer request - ${hostname}:${port}】\r\n----- req -----\r\n`,
							req,
							"\r\n----- res -----\r\n",
							res,
						);
				...
				fakeServer.on("listening", () => {
					...
					if (printDebugLog) {
						log.debug(
							`【fakeServer listening - ${hostname}:${port}】no arguments...`,
						);
					}
					resolve(serverObj);
				});
				fakeServer.on("upgrade", (req, socket, head) => {
					if (printDebugLog) {
						log.debug(
							`【fakeServer upgrade - ${hostname}:${port}】\r\n----- req -----\r\n`,
							req,
							"\r\n----- socket -----\r\n",
							socket,
							"\r\n----- head -----\r\n",
				...
				fakeServer.on("error", (e) => {
					log.error(
						`【fakeServer error - ${hostname}:${port}】\r\n----- error -----\r\n`,
						e,
					);
					if (!isListening) {
						reject(e);
				...
				fakeServer.on("clientError", (err, _socket) => {
					...
					log.error(
						`【fakeServer clientError - ${hostname}:${port}】\r\n`,
						err,
					);
					...
					if (port !== 443 && port !== 80) {
						if (ssl === true && err.code && err.code.startsWith("ERR_SSL_")) {
							...
							log.error(
								`自动兼容程序：SSL异常，现设置为禁用ssl: ${hostname}:${port}, ssl = false`,
							);
						} else if (ssl === false && err.code === "HPE_INVALID_METHOD") {
							compatible.setConnectSsl(hostname, port, true);
							log.error(
								`自动兼容程序：${err.code}，现设置为启用ssl: ${hostname}:${port}, ssl = true`,
							);
						}
					}
				});
				if (ssl) {
					fakeServer.on("tlsClientError", (err, _tlsSocket) => {
						...
						log.error(
							`【fakeServer tlsClientError - ${hostname}:${port}】\r\n`,
							err,
						);
					});
				}
				...
				if (printDebugLog) {
					if (ssl) {
						fakeServer.on("keylog", (line, tlsSocket) => {
							log.debug(
								`【fakeServer keylog - ${hostname}:${port}】\r\n----- line -----\r\n`,
								line,
								"\r\n----- tlsSocket -----\r\n",
								tlsSocket,
							);
						...
						fakeServer.on("secureConnection", (tlsSocket) => {
							log.debug(
								`【fakeServer secureConnection - ${hostname}:${port}】\r\n----- tlsSocket -----\r\n`,
								tlsSocket,
							);
						});
					}
					fakeServer.on("close", () => {
						log.debug(
							`【fakeServer close - ${hostname}:${port}】no arguments...`,
						);
					});
					fakeServer.on("connection", (socket) => {
						log.debug(
							`【fakeServer connection - ${hostname}:${port}】\r\n----- socket -----\r\n`,
							socket,
						);
					});
					fakeServer.on("checkContinue", (req, res) => {
						log.debug(
							`【fakeServer checkContinue - ${hostname}:${port}】\r\n----- req -----\r\n`,
							req,
							"\r\n----- res -----\r\n",
							res,
						);
					...
					fakeServer.on("checkExpectation", (req, res) => {
						log.debug(
							`【fakeServer checkExpectation - ${hostname}:${port}】\r\n----- req -----\r\n`,
							req,
							"\r\n----- res -----\r\n",
							res,
						);
					...
					fakeServer.on("connect", (req, socket, head) => {
						log.debug(
							`【fakeServer resumeSession - ${hostname}:${port}】\r\n----- req -----\r\n`,
							req,
							"\r\n----- socket -----\r\n",
							socket,
							"\r\n----- head -----\r\n",
