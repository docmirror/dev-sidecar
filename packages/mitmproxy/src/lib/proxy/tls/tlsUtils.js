...
async function generateForgeRsaKeyPair() {
	...
	const forgePrivateKey = pki.privateKeyFromAsn1(
		forge.asn1.fromDer(forge.util.createBuffer(privateKeyDer)),
	);
	const forgePublicKey = pki.rsa.setPublicKey(
		forgePrivateKey.n,
		forgePrivateKey.e,
	);
...
utils.isBrowserRequest = (userAgent) => /Mozilla/i.test(userAgent);

//
//  /^[^.]+\.a\.com$/.test('c.a.com')
//
const mappingHostNameRegexpCache = {};
utils.isMappingHostName = (DNSName, hostname) => {
	if (DNSName === hostname) {
		return true;
	}
...
utils.getMappingHostNamesFromCert = (cert) => {
...
// sync
utils.initCA = ({ caCertPath, caKeyPath }) => {
	try {
