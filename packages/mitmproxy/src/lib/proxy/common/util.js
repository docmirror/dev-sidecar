...
function getTimeoutConfig(hostname, serverSetting) {
	...
	const timeoutConfig =
		matchUtil.matchHostname(timeoutMapping, hostname, "get timeoutConfig") ||
		{};
	...
	return {
		...
		keepAliveTimeout:
			...
			serverSetting.defaultKeepAliveTimeout ||
			30000,
	};
}
...
function createHttpsAgent(timeoutConfig, verifySsl) {
	...
	if (!httpsAgentCache[key]) {
		...
		const checkServerIdentity = (host, cert) => {
			log.info(
				`checkServerIdentity: ${host}, CN: ${cert.subject.CN}, C: ${cert.subject.C || cert.issuer.C}, ST: ${cert.subject.ST || cert.issuer.ST}, bits: ${cert.bits}`,
			);
		};
		...
		log.info(
			"创建 HttpsAgent 成功, timeoutConfig:",
			timeoutConfig,
			", verifySsl:",
			verifySsl,
		);
