...
function _getConnectConfig(hostname, port) {
	...
	log.debug(
		`getConnectConfig: ${hostname}:${port}, ${jsonApi.stringify2(connectConfig)}`,
	);
...
function _getRequestConfig(hostname, port) {
	...
	log.debug(
		`getRequestConfig: ${hostname}:${port}, ${jsonApi.stringify2(requestConfig)}`,
	);
...
function _loadFromFile(defaultConfig) {
	...
	if (!fs.existsSync(configPath)) {
		...
		try {
			...
			log.error(
				"解析 automaticCompatibleConfig.json 失败:",
				configPath,
				", error:",
				e,
			);
...
function _saveConfigToFile() {
	...
	try {
		...
		log.error(
			"保存 automaticCompatibleConfig.json 失败:",
			filePath,
			", error:",
			e,
		);
...
module.exports = {
	...
	getConnectCompatibleConfig(hostname, port, manualCompatibleConfig = null) {
		let connectCompatibleConfig =
			manualCompatibleConfig == null
				? null
				: matchUtil.matchHostname(
						manualCompatibleConfig.connect,
						`${hostname}:${port}`,
						"getConnectCompatibleConfig",
					);
	...
	setConnectSsl(hostname, port, ssl, autoSave = true) {
		const connectCompatibleConfig = this.getConnectCompatibleConfig(
			hostname,
			port,
		);
	...
	getRequestCompatibleConfig(rOptions, manualCompatibleConfig = null) {
		let requestCompatibleConfig =
			manualCompatibleConfig == null
				? null
				: matchUtil.matchHostname(
						manualCompatibleConfig.request,
						`${rOptions.hostname}:${rOptions.port}`,
						"getRequestCompatibleConfig",
					);
		if (requestCompatibleConfig == null) {
			requestCompatibleConfig = _getRequestConfig(
				rOptions.hostname,
				rOptions.port,
			);
		}
	...
	setRequestRejectUnauthorized(rOptions, rejectUnauthorized, autoSave = true) {
		...
		if (requestCompatibleConfig) {
			...
			config.request[`${rOptions.hostname}:${rOptions.port}`] = {
				rejectUnauthorized,
			};
		}
		...
		log.info(
			`【自动兼容程序】${rOptions.hostname}:${rOptions.port}: 设置 request.rejectUnauthorized = ${rejectUnauthorized}`,
		);
	},
};
