...
class DynamicChoice {
	constructor(key) {
		this.key = key;
		this.countMap =
			{}; /* ip -> count { value, total, error, keepErrorCount, successRate }  */
	...
	changeNext(count) {
		...
		if (this.backupList.length > 0) {
			this.value = this.backupList.shift();
			log.info(
				`切换backup完成: ${this.key}, ip: ${valueBackup} ➜ ${this.value}, this:`,
				this,
			);
		} else {
			this.value = null;
			log.info(
				`切换backup完成: ${this.key}, backupList为空了，设置this.value: from '${valueBackup}' to null. this:`,
				this,
			);
		}
	}
