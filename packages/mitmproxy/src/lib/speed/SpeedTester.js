...
const {
	configFromFiles,
} = require("@docmirror/dev-sidecar/src/config/index.js");

const familyMapping = matchUtil.domainMapRegexply(
	configFromFiles.server.dns.familyMapping,
);
...
class SpeedTester {
	...
	async test() {
		...
		this.testCount++;
		log.debug(
			`[speed] test start: ${this.hostname}, testCount: ${this.testCount}`,
		);
		...
		try {
			...
			log.info(
				`[speed] test end: ${this.hostname} ➜ ip-list:`,
				this.backupList,
			...
			log.error(
				`[speed] test failed: ${this.hostname}, testCount: ${this.testCount}, error:`,
				e,
			);
		} finally {
			this.isTesting = false;
	...
	async testBackups() {
		if (this.isTestingBackups) {
			log.debug(
				`[speed] testBackups skipped (already running): ${this.hostname}`,
			);
			return;
		}
	...
	async _doTest(item, aliveList) {
		try {
			...
			log.info(
				`[speed] test success: ${this.hostname} ➜ ${item.host}:${this.port} from DNS '${item.dns}'`,
			);
			_.merge(item, ret);
	...
	testByTCP(item) {
		return new Promise((resolve, reject) => {
			...
			const client = net.createConnection(
				{ host, port: this.port, family: host.includes(":") ? 6 : 4 },
				() => {
					isOver = true;
					...
					resolve({
						status: "success",
						by: "TCP",
						target: `${host}:${this.port}`,
						time: connectionTime - startTime,
					});
					client.end();
				},
			);
			client.on("error", (e) => {
				...
				log.warn(
					...
					this.hostname,
					`➜ ${host}:${this.port} from DNS '${dns}', cost: ${Date.now() - startTime} ms, errorMsg:`,
					e.message,
				);
				reject(e);
				client.destroy();
			...
			timeoutId = setTimeout(() => {
				if (!isOver) {
					...
					log.warn(
						...
						this.hostname,
						`➜ ${host}:${this.port} from DNS '${dns}', cost: ${Date.now() - startTime} ms`,
					);
		...
		return new Promise((resolve, reject) => {
			...
			this.testByTCP(item)
				...
				.catch((e) => {
					...
					reject(
						new Error(`TCP测速失败：${item.host}:${this.port} ${e.message}`),
					);
				});
		});
	}
