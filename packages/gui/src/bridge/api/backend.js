...
function getMetaInfo(config, fallbackId) {
	const metaInfo =
		lodash.get(config, "app.metaInfo") || lodash.get(config, "metaInfo") || {};
	return {
...
const localApi = {
	...
	info: {
		get() {
			...
			const personalRemote = getMetaInfo(
				configLoader.getRemoteConfig("_personal"),
				"",
			);
				...
				configProfiles: {
					...
					personalRemote: {
						...
						enabled:
							...
							Boolean(remoteConfig.personalUrl),
					},
				},
			};
		},
		...
		getLogDir() {
			return (
				configFromFiles.app.logFileSavePath ||
				path.join(getDefaultConfigBasePath(), "/logs/")
			);
		},
