	...
	if (process.env.NODE_ENV === "development") {
		...
		if (isMac) {
			autoUpdater.updateConfigPath = path.join(
				__dirname,
				"mac/dev-sidecar.app/Contents/Resources/app-update.yml",
			);
		} else if (isLinux) {
			autoUpdater.updateConfigPath = path.join(
				__dirname,
					...
					if (response && response.statusCode === 200) {
						...
						try {
							...
							win.webContents.send("update", {
								key: "error",
								action: "checkForUpdate",
								error: "检查更新失败，github API返回数据格式不正确",
							});
							return;
						}
						...
						if (typeof data !== "object" || data.length === undefined) {
							...
							win.webContents.send("update", {
								key: "error",
								action: "checkForUpdate",
								error: "检查更新失败，github API返回数据不是数组",
							});
							return;
						}

						log.debug(
							"github api返回的release数据：",
						...
						for (let i = 0; i < data.length; i++) {
							...
							if (!onlineVersion) {
								log.info(
									"跳过无法提取版本号的版本:",
									versionData.name || versionData.tag_name,
								);
								continue; // 跳过即 “不是正式，又不是预发布” 的版本
							}

							const isOnlinePreRelease =
								onlineVersion.includes("-") || versionData.prerelease;
							if (
								!isCurrentPreRelease &&
								DevSidecar.api.config.get().app.skipPreRelease &&
								isOnlinePreRelease
							) {
								log.info(
									"跳过预发布版本:",
							...
							log.info(
								`版本比对结果：isNewVersion('${onlineVersion}', '${curVersion}') = ${isNew}`,
							);
							if (isNew > 0) {
								log.info(
									`检查更新：发现新版本 '${onlineVersion}'，当前版本号为 '${curVersion}'`,
								);
								win.webContents.send("update", {
									key: "available",
									value: {
										...
										releaseNotes: versionData.body
											? versionData.body
													.replace(/\r\n/g, "\n")
													.replace(
														/https:\/\/github.com\/docmirror\/dev-sidecar/g,
														"",
													)
													.replace(/(?<=(^|\n))[ \t]*(?:#[ #]*)?#\s*/g, "") ||
												"无"
								...
								log.info(
									`检查更新：没有新版本，最近发布的版本号为 '${onlineVersion}'，而当前版本号为 '${curVersion}'`,
								);
