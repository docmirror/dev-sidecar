...
export default {
	...
	methods: {
		...
		async doCheckRootCa() {
			...
			if (
				this.setting.rootCa &&
				(this.setting.rootCa.setuped || this.setting.rootCa.noTip)
			) {
				return;
			}
			this.$confirm({
		...
		async handleCaSetuped() {
			...
			await this.$api.shell.setupCa({
				certPath: this.config.server.setting.rootCaFile.certPath,
			});
			this.setting.rootCa = this.setting.rootCa || {};
			const rootCa = this.setting.rootCa;
		...
		createSwitchBtns() {
			...
			btns.server = this.createSwitchBtn(
				"server",
				"代理服务",
				this.$api.server,
				status,
			);
			btns.proxy = this.createSwitchBtn(
				"proxy",
				"系统代理",
				this.$api.proxy,
				status,
			...
			lodash.forEach(status.plugin, (_item, key) => {
				...
				btns[key] = this.createSwitchBtn(
					key,
					this.config.plugin[key].name,
					this.$api.plugin[key],
					status.plugin,
					this.config.plugin[key].tip,
				);
			});
			return btns;
		},
