...
let hideDockWhenWinClose =
	DevSidecar.api.config.get().app.dock.hideWhenWinClose || false;
...
function checkHideWin() {
	...
	if (isLinux && !hasShortcut(config.app.showHideShortcut)) {
		dialog.showMessageBox({
			...
			title: "提示：请先设置快捷键",
			message:
				"由于大部分 Linux 系统没有系统托盘，所以需使用快捷键呼出窗口。\n但您还未设置快捷键，请先到 “设置” 页面中设置好快捷键，再关闭窗口。",
			buttons: ["确定"],
		});
		return false;
	}
...
function createWindow(startHideWindow, autoQuitIfError = true) {
	...
	win.on("close", (e, ...args) => {
		...
		if (closeStrategy === 1) {
			...
			win.webContents.send("close.showTip", {
				closeStrategy,
				showHideShortcut: config.app.showHideShortcut,
			});
		}
	});
	...
	win.webContents.on("before-input-event", (event, input) => {
		win.webContents
			.executeJavaScript("config")
			.then((value) => {
				console.info("window.config:", value, ", key:", input.key);
				if (
					!value ||
					(value.disableBeforeInputEvent !== true &&
						value.disableBeforeInputEvent !== "true")
				) {
...
function registerShowHideShortcut(showHideShortcut) {
	...
	if (hasShortcut(showHideShortcut)) {
		try {
			const registerSuccess = globalShortcut.register(
				DevSidecar.api.config.get().app.showHideShortcut,
				() => {
					if (winIsHidden) {
						showWin();
					} else {
...
try {
	...
	log.info(
		...
		jsonApi.stringify2(app.getLoginItemSettings()),
	);
		...
		app.on("ready", async () => {
			if (isWindows) {
				try {
					...
					log.error(
						`加载 './background/powerMonitor' 失败，现捕获异常并使用默认的 powerMonitor。\r\n目前，启动着DS重启电脑时，将无法正常关闭系统代理，届时请自行关闭系统代理！\r\n捕获的异常信息:`,
						e,
					);
				}
			}
